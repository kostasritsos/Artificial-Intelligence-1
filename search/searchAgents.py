...
class CornersProblem(search.SearchProblem):
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state) :
            ...
            "*** YOUR CODE HERE ***"
            new=self.getNextState(state,action)
            Cost=self.getActionCost(state,action,new)
            children.append((new,action,Cost))
    ...
    def getNextState(self, state, action):
        ...
        TVisited=tuple(Visited)
        return((nextx,nexty),TVisited)
        #util.raiseNotDefined()
...
def cornersHeuristic(state, problem):
    ...
    while len(unvisited_corners)!=0:
        ...
        for corner in unvisited_corners:
            if util.manhattanDistance(node, corner)<Min:
                Min=util.manhattanDistance(node,corner)
                Corner=corner
...
def foodHeuristic(state, problem):
    ...
    Distances=[0]
    for j in Food:
        Distances.append(util.manhattanDistance(position,j))
    heuristic=heuristic+max(Distances)
    return heuristic
